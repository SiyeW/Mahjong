...
def print_shanten_14(hc: str):
    ...
    for x in card_advice_list:
        print(f"打{convert_num_to_card(x[0])} 共{x[2]}枚 待", "".join([convert_num_to_card(x) for x in x[1]]), sep="")
    if not xt:
        print("出现错误")
        
def print_shanten_13(hc: str):
    print(hc) # 输出手牌
    hc_list = convert_hc_to_list(hc)
    xt, xt_list = calc_shanten_13(hc_list = hc_list)
    msum = calc_tenpai_sum(hc_list, xt_list)
    xt_text = "听牌" if xt == 0 else f"{xt}向听"
    print(f"{xt_text} 共{msum}枚 待", "".join([convert_num_to_card(x) for x in xt_list]), sep="")